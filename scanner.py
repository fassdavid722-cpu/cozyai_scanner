...
BITGET_API_KEY = os.getenv("BITGET_API_KEY", "") BITGET_SECRET = os.getenv("BITGET_SECRET", "") BITGET_PASSWORD = os.getenv("BITGET_PASSWORD", "")

TELEGRAM_TOKEN = os.getenv("TELEGRAM_TOKEN", "") CHAT_ID = os.getenv("CHAT_ID", "")

DRY_RUN = os.getenv("DRY_RUN", "true").lower() == "true" LIVE_TRADING = os.getenv("LIVE_TRADING", "false").lower() == "true" USE_DEMO_TRADING = os.getenv("USE_DEMO_TRADING", "true").lower() == "true"

STARTING_EQUITY = float(os.getenv("STARTING_EQUITY", "3.0")) MAX_DAILY_LOSS_PCT = float(os.getenv("MAX_DAILY_LOSS_PCT", "0.05")) MAX_CONSECUTIVE_LOSSES = int(os.getenv("MAX_CONSECUTIVE_LOSSES", "3")) MAX_LEVERAGE = float(os.getenv("MAX_LEVERAGE", "10.0")) MIN_LEVERAGE = float(os.getenv("MIN_LEVERAGE", "5.0")) DEFAULT_LEVERAGE = float(os.getenv("DEFAULT_LEVERAGE", "5.0")) MIN_NOTIONAL_EXCHANGE = float(os.getenv("MIN_NOTIONAL_EXCHANGE", "5.0")) RISK_PER_TRADE_PCT = float(os.getenv("RISK_PER_TRADE_PCT", "0.02")) BOOTSTRAP_THRESHOLD = float(os.getenv("BOOTSTRAP_THRESHOLD", "10.0")) BASE_THRESHOLD = float(os.getenv("BASE_THRESHOLD", "0.68")) MAX_SYMBOLS = int(os.getenv("MAX_SYMBOLS", "20")) TIMEFRAME = os.getenv("TIMEFRAME", "1m") OHLCV_LIMIT = int(os.getenv("OHLCV_LIMIT", "60")) FAST_ENTRY_SCORE = float(os.getenv("FAST_ENTRY_SCORE", "0.78")) HISTORY_LIMIT = int(os.getenv("HISTORY_LIMIT", "200")) TAKE_PROFIT_R = float(os.getenv("TAKE_PROFIT_R", "2.0")) MIN_SIGNAL_SCORE = float(os.getenv("MIN_SIGNAL_SCORE", "0.65")) MAX_HOLD_MINUTES = int(os.getenv("MAX_HOLD_MINUTES", "30"))

SPREAD_PCT = float(os.getenv("SPREAD_PCT", "0.0005")) TAKER_FEE_PCT = float(os.getenv("TAKER_FEE_PCT", "0.0006")) SLIPPAGE_PCT = float(os.getenv("SLIPPAGE_PCT", "0.0003"))

MIN_ATR_PCT = float(os.getenv("MIN_ATR_PCT", "0.0005")) MIN_VOLUME_SPIKE = float(os.getenv("MIN_VOLUME_SPIKE", "0.0")) CONFIDENCE_DECAY = float(os.getenv("CONFIDENCE_DECAY", "0.05"))

HEARTBEAT_INTERVAL_RUNS = int(os.getenv("HEARTBEAT_INTERVAL_RUNS", "6")) MARKET_PULSE_INTERVAL_RUNS = int(os.getenv("MARKET_PULSE_INTERVAL_RUNS", "12")) STALL_ALERT_AFTER_MINUTES = int(os.getenv("STALL_ALERT_AFTER_MINUTES", "15"))

DRIVE_FOLDER_ID = os.getenv("DRIVE_FOLDER_ID", "1Ox77rDeIj7XEE_pyfE5TyKVtiYtbdcXe") GDRIVE_CREDS_JSON = os.getenv("GDRIVE_CREDS", "") LOG_TO_DRIVE = bool(GDRIVE_CREDS_JSON)

MEMORY_FILE = os.getenv("COZY_MEMORY_FILE", "cozy_memory.json") LOCAL_TRADE_LOG = os.getenv("LOCAL_TRADE_LOG", "trade_log_local.csv") RAW_SETUPS_LOG = os.getenv("RAW_SETUPS_LOG", "raw_setups.csv") LOG_FILE = os.getenv("ALPHA_LOG_FILE", "cozy_scanner.log")

STRATEGY_LIST = [ "volume_breakout", "orderbook_imbalance", "funding_arbitrage", "liquidity_sweep", "momentum", ] DEFAULT_STRATEGY_WEIGHTS = {s: 1.0 / len(STRATEGY_LIST) for s in STRATEGY_LIST}
...
def log(msg: str) -> None: line = f"{datetime.utcnow().isoformat()} | {msg}" print(line) try: with open(LOG_FILE, "a", encoding="utf-8") as f: f.write(line + "\n") except Exception: pass
...
def send_telegram(message: str) -> None: log(message) if not TELEGRAM_TOKEN or not CHAT_ID: return try: url = f"https://api.telegram.org/bot{TELEGRAM_TOKEN}/sendMessage" requests.post(url, data={"chat_id": CHAT_ID, "text": message}, timeout=10) except Exception as exc: log(f"Telegram error: {exc}")
...
def get_drive_service(): if not LOG_TO_DRIVE or service_account is None or build is None: return None try: creds_dict = json.loads(GDRIVE_CREDS_JSON) creds = service_account.Credentials.from_service_account_info( creds_dict, scopes=["https://www.googleapis.com/auth/drive"] ) return build("drive", "v3", credentials=creds) except Exception as exc: log(f"Drive auth error: {exc}") return None

def download_file(drive, filename: str, folder_id: str): if not drive or MediaIoBaseDownload is None: return None try: query = f"name='{filename}' and '{folder_id}' in parents and trashed=false" res = drive.files().list(q=query, fields="files(id)").execute() files = res.get("files", []) if not files: return None file_id = files[0]["id"] request = drive.files().get_media(fileId=file_id) fh = BytesIO() downloader = MediaIoBaseDownload(fh, request) done = False while not done: _, done = downloader.next_chunk() fh.seek(0) return fh except Exception as exc: log(f"Download error {filename}: {exc}") return None

def upload_file(drive, filename: str, folder_id: str, content_bytes: bytes): if not drive or MediaIoBaseUpload is None: return try: query = f"name='{filename}' and '{folder_id}' in parents and trashed=false" res = drive.files().list(q=query, fields="files(id)").execute() files = res.get("files", []) media = MediaIoBaseUpload(BytesIO(content_bytes), mimetype="text/csv", resumable=True) if files: drive.files().update(fileId=files[0]["id"], media_body=media).execute() else: metadata = {"name": filename, "parents": [folder_id]} drive.files().create(body=metadata, media_body=media).execute() except Exception as exc: log(f"Upload error {filename}: {exc}")

def append_to_csv(drive, filename: str, folder_id: str, new_rows_df: pd.DataFrame): if new_rows_df is None or new_rows_df.empty: return try: if drive and MediaIoBaseDownload is not None: query = f"name='{filename}' and '{folder_id}' in parents and trashed=false" res = drive.files().list(q=query, fields="files(id)").execute() files = res.get("files", []) if files: file_id = files[0]["id"] request = drive.files().get_media(fileId=file_id) fh = BytesIO() downloader = MediaIoBaseDownload(fh, request) done = False while not done: _, done = downloader.next_chunk() fh.seek(0) old_df = pd.read_csv(fh) df = pd.concat([old_df, new_rows_df], ignore_index=True) else: df = new_rows_df buf = BytesIO() df.to_csv(buf, index=False) upload_file(drive, filename, folder_id, buf.getvalue()) return except Exception as exc: log(f"Append to Drive error: {exc}")

# Local fallback if Drive unavailable
try:
...
def safe_call(func, retries: int = 3, delay: float = 1.5): last_exc = None for _ in range(retries): try: return func() except Exception as exc: last_exc = exc time.sleep(delay) if last_exc: raise last_exc return None

def apply_friction(price: float, is_entry: bool) -> float: friction = SPREAD_PCT + TAKER_FEE_PCT + SLIPPAGE_PCT return price * (1 + friction) if is_entry else price * (1 - friction)

def make_id(prefix: str) -> str: return f"{prefix}_{int(time.time())}"
...
class Memory: def init(self, drive, folder_id: str): self.drive = drive self.folder_id = folder_id self.file = MEMORY_FILE self.data = self.load() self.normalize()
...
def record_loss(self, symbol: str) -> None:
    ...
    self.data["learned_biases"][symbol] = clamp(self.data["learned_biases"].get(symbol, 0.0) + 0.05, 0.0, 0.25)
    self.save()

def record_trade(self, symbol: str, side: str, score: float, lev: float, pnl_pct: float, strategy_used: str, regime: str, hour: int) -> None:
    trade = {
...
def apply_learning(self) -> None:
    ...
    for sym, avg in sym_perf.items():
        if avg < -1.0:
            self.data["learned_biases"][sym] = clamp(self.data["learned_biases"].get(sym, 0.0) + 0.05, 0.0, 0.25)
...
class MarketData: def init(self, exchange): self.exchange = exchange
...
def compute_volume_breakout(df: pd.DataFrame) -> Tuple[float, str]: if len(df) < 20: return 0.5, "neutral" data = df.copy() data["range"] = (data["high"] - data["low"]).replace(0, 1e-9) data["v_intensity"] = data["volume"] / data["range"] mean = data["v_intensity"].mean() std = data["v_intensity"].std() + 1e-9 z = (data["v_intensity"].iloc[-1] - mean) / std prob = safe_sigmoid(z) if prob > 0.70: return float(prob), "long" if prob < 0.30: return float(1.0 - prob), "short" return 0.5, "neutral"
...
def compute_liquidity_sweep(df: pd.DataFrame) -> Tuple[float, str]: if len(df) < 30: return 0.5, "neutral" recent = df.tail(30).copy() recent["high_20"] = recent["high"].rolling(20).max() recent["low_20"] = recent["low"].rolling(20).min() recent["sweep_high"] = (recent["high"] > recent["high_20"].shift(1)) & (recent["close"] < recent["high"]) recent["sweep_low"] = (recent["low"] < recent["low_20"].shift(1)) & (recent["close"] > recent["low"]) sweep = recent[recent["sweep_high"] | recent["sweep_low"]] if sweep.empty: return 0.5, "neutral" last = sweep.iloc[-1] if bool(last["sweep_low"]): return 0.70, "long" return 0.70, "short"

def compute_momentum(df: pd.DataFrame) -> Tuple[float, str]: if len(df) < 26: return 0.5, "neutral" close = df["close"].copy() delta = close.diff() up = delta.clip(lower=0) down = -delta.clip(upper=0) avg_up = up.rolling(14).mean().iloc[-1] avg_down = down.rolling(14).mean().iloc[-1] rsi = 100 if avg_down == 0 else 100 - (100 / (1 + (avg_up / avg_down))) exp12 = close.ewm(span=12, adjust=False).mean() exp26 = close.ewm(span=26, adjust=False).mean() macd = exp12 - exp26 signal = macd.ewm(span=9, adjust=False).mean() hist = macd - signal if rsi > 70 and hist.iloc[-1] < 0: return 0.70, "short" if rsi < 30 and hist.iloc[-1] > 0: return 0.70, "long" return 0.5, "neutral"

def detect_regime(df: pd.DataFrame) -> str: if len(df) < 30: return "ranging" high = df["high"] low = df["low"] close = df["close"] plus_dm = high.diff() minus_dm = low.diff() plus_dm[plus_dm < 0] = 0 minus_dm[minus_dm > 0] = 0 tr = pd.concat([high - low, (high - close.shift(1)).abs(), (low - close.shift(1)).abs()], axis=1).max(axis=1) atr = tr.rolling(14).mean() plus_di = 100 * (plus_dm.rolling(14).mean() / (atr + 1e-9)) minus_di = 100 * (minus_dm.abs().rolling(14).mean() / (atr + 1e-9)) dx = 100 * (plus_di - minus_di).abs() / ((plus_di + minus_di) + 1e-9) adx = dx.rolling(14).mean().iloc[-1] close_series = close.tail(20) net_move = abs(close_series.iloc[-1] - close_series.iloc[0]) total_move = close_series.diff().abs().sum() efficiency = net_move / total_move if total_move != 0 else 0.0 if adx > 25 and efficiency > 0.3: return "trending" return "ranging"

def estimate_stop_distance_pct(df: pd.DataFrame) -> float: prev_close = df["close"].shift(1) tr = pd.concat([ df["high"] - df["low"], (df["high"] - prev_close).abs(), (df["low"] - prev_close).abs(), ], axis=1).max(axis=1) atr = tr.rolling(14).mean().iloc[-1] price = float(df["close"].iloc[-1]) return max((atr * 1.2) / price, MIN_ATR_PCT)

def get_atr_pct(df: pd.DataFrame) -> float: prev_close = df["close"].shift(1) tr = pd.concat([ df["high"] - df["low"], (df["high"] - prev_close).abs(), (df["low"] - prev_close).abs(), ], axis=1).max(axis=1) atr = tr.rolling(14).mean().iloc[-1] price = float(df["close"].iloc[-1]) return float(atr / price) if price > 0 else 0.0

def compute_volume_spike(df: pd.DataFrame) -> float: if len(df) < 20: return 1.0 avg_vol = df["volume"].rolling(20).mean().iloc[-1] last_vol = df["volume"].iloc[-1] return float(last_vol / avg_vol) if avg_vol > 0 else 1.0

def compute_liquidity_quality(df: pd.DataFrame, imbalance: float, volume_spike: float) -> float: score = 0.5 atr_pct = get_atr_pct(df) if volume_spike > 1.5: score += 0.2 elif volume_spike < 1.0: score -= 0.2 if imbalance > 1.3 or imbalance < 0.7: score += 0.2 else: score -= 0.1 if 0.001 < atr_pct < 0.01: score += 0.2 else: score -= 0.1 return clamp(score, 0.0, 1.0)

======================================================

DIAGNOSTICS / NEEDS ANALYZER

======================================================

class MarketNeedsAnalyzer: def analyze(self, df: pd.DataFrame, symbol: str, imbalance: float, atr_pct: float, volume_spike: float) -> List[str]: needs = [] if atr_pct < 0.001: needs.append("Low volatility → waiting for liquidity expansion") if volume_spike < 1.2: needs.append("Weak volume participation → no institutional interest") body = abs(df["close"].iloc[-1] - df["open"].iloc[-1]) candle_range = (df["high"].iloc[-1] - df["low"].iloc[-1]) + 1e-9 if body / candle_range < 0.3: needs.append("Indecision candle → no directional displacement") if 0.8 < imbalance < 1.2: needs.append("Neutral orderbook → no directional pressure") return needs
...
@dataclass class TradeIntent: id: str symbol: str mode: str  # immediate | watch direction: str  # long | short trigger: str  # breakout | dump | momentum | sweep | breakdown | manual take_profit_pct: Optional[float] = None stop_loss_pct: Optional[float] = None leverage: float = DEFAULT_LEVERAGE risk_level: str = "medium" note: str = "" created_at: str = "" active: bool = True
...
class TradeLifecycle: def init(self, memory: Memory, market: MarketData, exchange, drive): self.memory = memory self.market = market self.exchange = exchange self.drive = drive self._finalized_ids = set()
...
class SignalEngine: def init(self, memory: Memory): self.memory = memory self.needs_analyzer = MarketNeedsAnalyzer() self.rejection_reasons: List[str] = [] self.last_regime = "unknown"
...
class RiskEngine: def init(self, memory: Memory): self.memory = memory
...
class ExecutionEngine: def init(self, memory: Memory, exchange): self.memory = memory self.exchange = exchange
...
class Observability: def init(self, memory: Memory): self.memory = memory
...
class CozyHybridAI: def init(self): self.drive = get_drive_service() if LOG_TO_DRIVE else None self.memory = Memory(self.drive, DRIVE_FOLDER_ID) self.exchange = ccxt.bitget({ "apiKey": BITGET_API_KEY, "secret": BITGET_SECRET, "password": BITGET_PASSWORD, "options": {"defaultType": "swap"}, "enableRateLimit": True, "timeout": 20000, }) if USE_DEMO_TRADING: try: self.exchange.enable_demo_trading(True) except Exception: pass try: if DRY_RUN: self.exchange.set_sandbox_mode(True) except Exception: pass try: self.exchange.load_markets() except Exception: pass
...
if name == "main": print("DEBUG: Before creating AI instance") ai = CozyHybridAI() print("DEBUG: AI instance created, running") ai.run() print("DEBUG: Run finished")
